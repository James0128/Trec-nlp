...
NUM_LABELS = int(parser.get("Parameters", "NUM_LABELS"))
...
build_random_vocabulary(word_appear_times)
...
def spliteDataset(validation_size):
    ...
    test_set = [x for x in data_set if x not in train_set]
    return train_set,test_set
train_set,test_set=spliteDataset(validation_size)
...
# 5. traning step: two model: sentence to words then words to vectors
# 5.1 bow model
...
# 6. testing step and evaluate model  有错误 需要修改
precision_list=[]
def save_output(path_eval_result,label_to_ix, precision_list):
    with open(path_eval_result,'w') as f:
        for i in range(len(label_to_ix)):
            f.write(list(label_to_ix.keys())[i]+'\t\t'+str(precision_list[i]))
            f.write('\n')
        f.close()
...
def bow_test():
    ...
    with torch.no_grad():
        ...
        for i in range(rows):
            ...
            recall_mean+=recall
        recall_mean = float(recall_mean / len(label_to_ix))
        for i in range(cols):
            total_cols=0
            for j in range(rows):
            ...
            precision_mean+=pre
        save_output(path_eval_result,label_to_ix,precision_list)
        precision_mean = float(precision_mean / len(label_to_ix))
...
def bilstm_ensemble_train(models, trn, model_num):
    print('Bilstm ensemble model is training....')
    for i in range(model_num):
        ...
        for epoch in range(epoches):
            for sentence, tags in models.bagging(train_set):
                ...
                loss.backward()
                bilstm_optimizer.step()
        torch.save(model, '../data/bilstm_ensemble_trained_model' + str(epoch) + '.pt')
        print('Bilstm ensemble-', epoch, 'ends')
        pass
...
# 8. Determine input command: train and test which model
def select_operation():
    if sys.argv[1] ==  'train':
        if model=='bow':
            ...
            bilstm_model = BiLSTMClassifier(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(label_to_ix))
            bilstm_train(bilstm_model)
        elif model=='bilstm_ensemble':
            ensemble_size = int(parser.get("Parameters", "ensemble_size"))
            bilstm_ensemble_model = BiLSTMEnsembleClassifier(ensemble_size,EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(label_to_ix))
